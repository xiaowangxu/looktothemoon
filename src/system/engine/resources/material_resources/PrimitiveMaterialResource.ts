...
export class FlatMaterialResource extends MaterialResource {
    ...
    bool is_in_shadow(const in vec3 world_pos, const in mat4 projection, const in float bias) {
        vec4 projected_pos = projection * vec4(world_pos, 1.0);
    ...
    void main() {
        ...
        for(int i = 0; i < max_count; i++) {
            ...
            light.attenuation *= mix(1.0, shadow, light.shadow_opacity);
            i += light.stride;
    ...
    static readonly #fragment_shade_uniforms: UniformInitSet<WebGL2RenderState> = {
        ...
        shadows: { type: RenderStateUniformType.Int, default: RenderServerDevice.ShadowsTextureUnit },
        sky: { type: RenderStateUniformType.Int, default: RenderServerDevice.SkyTextureUnit },
        u_color: { type: RenderStateUniformType.Vec4, default: Color.new },
    };

    public get uniforms() { return FlatMaterialResource.#uniforms; }
    ...
    public update_Material() {
        ...
        const vertex_shader = PrimitiveVertexShader.get(this.config).expect;
        const fragment_prez_shader = PrimitiveFragmentPreZShader.get(this.config).expect;
        const fragment_shade_shader = this.render_server.render_state.create_Shader(RenderStateShaderType.Fragment, FlatMaterialResource.#fragment_shade_shader).expect();
        shader.set_Shaders(
            vertex_shader,
            PrimitiveVertexShaderUniforms,
            {
                ...
                shade: {
                    shader: fragment_shade_shader,
                    uniforms: FlatMaterialResource.#fragment_shade_uniforms,
                },
            }
        );
        this.material.set_Material(shader, FlatMaterialResource.#uniforms);
        this.material.transparent = false;
    }
}
