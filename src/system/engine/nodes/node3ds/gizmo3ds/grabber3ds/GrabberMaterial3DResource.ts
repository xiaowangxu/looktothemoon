...
const GrabberMaterial3DUniformLayout = new RefCacher(() => {
...
const GrabberMaterial3DPipelineCacheSet = new RefCacher(() => {
...
`,
	...
	var color = mat_uniform.color * vary.color;`,
		// custom
		undefined,
		GrabberMaterial3DUniformLayout.get(),
...
export class GrabberMaterial3DResource extends MaterialResource {
	...
	private readonly uniform_group_ref = new ReadonlyRef(RenderServer.render_state.create_UniformGroup(GrabberMaterial3DUniformLayout.get()).expect());
	private readonly uniform_buffer_ref = new ReadonlyRef(RenderServer.render_state.create_Buffer(WebGPURenderStateBufferType.Uniform, WebGPURenderStateBufferUsage.CopyDst, GrabberMaterial3DResource.UniformMemoryLayout.size, false).expect());
	private readonly uniform_array_buffer = new ArrayBuffer(GrabberMaterial3DResource.UniformMemoryLayout.size);
	...
	constructor() {
		...
		GrabberMaterial3DPipelineCacheSet.get().set_RenderServerMaterialPipelineCaches(this.render_server_material, this.uniform_group_ref.expect);
		this.update_UniformBuffer();
	}
