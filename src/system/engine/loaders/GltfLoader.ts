...
import { WebGPURenderElementTextureSamplerCacheHash as SamplerHash } from "@/system/sliverofstraw/render_element_object/texture_sampler/WebGPURenderElementTextureSamplerCache";
import { WebGPURenderStateTextureWrap } from "@/system/sliverofstraw/render_state_object/texture/WebGPURenderStateTextureSampler";
import { RenderServerGeometryAttributeLayoutBuffer } from "../render_server/geometry/RenderServerGeometryDefination";
import { bitmask_check, bitmask_enable } from "@/system/utils/BitMask";
...
const mat = new RefCacher(() => {
    const pbr = new PbrMaterial3DResource();
    pbr.color = Vector4.create(0.75, 0.75, 0.75, 1.0);
    pbr.metallic = 0;
    pbr.roughness = 1;
    return pbr;
});
...
export class GltfLoader {
    ...
    static #tmp_quaternion_0 = Quaternion.new;
    ...
    private static readonly ComponentTypes: { [key: number]: (new (data: number | ArrayBuffer, offset?: number, length?: number) => ArrayBufferView) & { BYTES_PER_ELEMENT: number } } = {
        5120: Int8Array,
        5121: Uint8Array,
        5122: Int16Array,
        5123: Uint16Array,
    ...
    private static readonly FilterTypes: { [key: number]: FilterType } = {
        9728: FilterType.Nearest,
        9729: FilterType.Linear,
        9984: FilterType.NearestMipmapNearest,
        9985: FilterType.LinearMipmapNearest,
        9986: FilterType.NearestMipmapLinear,
        9987: FilterType.LinearMipmapLinear,
    };
    private static readonly WrapModes: { [key: number]: WebGPURenderStateTextureWrap } = {
        33071: WebGPURenderStateTextureWrap.Clamp,// ClampToEdgeWrapping,
        33648: WebGPURenderStateTextureWrap.Repeat,// MirroredRepeatWrapping,
        10497: WebGPURenderStateTextureWrap.MirrorRepeat,// RepeatWrapping,
    };
    ...
    private readonly accessor_cache: Map<number, ArrayBufferView> = new Map();
    private parse_Accessor(index: number): ArrayBufferView {
        ...
        if (accessor.bufferView === undefined && accessor.sparse === undefined) {
            const item_size = GltfLoader.TypeSizes[accessor.type];
            const TypedArray = GltfLoader.ComponentTypes[accessor.componentType];
            const normalized = accessor.normalized === true;
            const array = new TypedArray(accessor.count * item_size);
            this.accessor_cache.set(index, array);
            return array;
        }
    ...
    private create_MeshGroup(primitive: any): MeshGroup {
        const attributes: MeshGroup = {
            position: undefined,
            normal: undefined,
            uv: undefined,
            tangent: undefined,
        ...
        for (const attr_name in primitive.attributes) {
            if (attr_name === Attributes.POSITION || attr_name === Attributes.NORMAL || attr_name === Attributes.TEXCOORD_0 || attr_name === Attributes.TANGENT || attr_name === Attributes.COLOR_0) {
                ...
                switch (attr_name) {
                    case Attributes.POSITION: {
                        attributes.position = buffer_attribute as Float32Array;
                        attributes.attributes = bitmask_enable(attributes.attributes, RenderServerGeometryAttributeLayoutBuffer.PositionNormal);
                        break;
                    }
                    case Attributes.NORMAL: {
                        attributes.normal = buffer_attribute as Float32Array;
                        break;
                    }
                    case Attributes.TANGENT: {
                        attributes.tangent = buffer_attribute as Float32Array;
                        attributes.attributes = bitmask_enable(attributes.attributes, RenderServerGeometryAttributeLayoutBuffer.Tangent);
                        break;
                    }
                    case Attributes.TEXCOORD_0: {
                        attributes.uv = buffer_attribute as Float32Array;
                        attributes.attributes = bitmask_enable(attributes.attributes, RenderServerGeometryAttributeLayoutBuffer.Uv);
                        break;
                    }
                    case Attributes.COLOR_0: {
                        attributes.color = buffer_attribute as Float32Array;
                        attributes.attributes = bitmask_enable(attributes.attributes, RenderServerGeometryAttributeLayoutBuffer.Color);
                        break;
                    }
                }
            }
        ...
        if (primitive.indices !== undefined) {
            const index = this.parse_Accessor(primitive.indices);
            if (index instanceof Uint16Array) {
                attributes.index = new Uint32Array(index);
            }
            else {
                attributes.index = index as Uint32Array;
            }
        }
        return attributes;
    }

    private create_Mesh(primitives: any[]) {
        const groups: MeshGroup[] = [];
        for (let i = 0, count = primitives.length; i < count; i++) {
            const primitive = primitives[i];
            if (primitive.extensions && primitive.extensions[GltfLoader.Extensions.KHR_DRACO_MESH_COMPRESSION]) throw new Error('DRACO geometry is not available');
            else {
                const group = this.create_MeshGroup(primitive);
                groups.push(group);
            }
        }

        // merge geometry
        if (groups.length === 0) throw new Error('mesh has no primitives');
        groups.sort((a, b) => a.id - b.id);
        const attribute = groups[0].attributes;
        const has_index = groups[0].index !== undefined;
        if (groups.length > 1) {
            for (let i = 1; i < groups.length; i++) {
                if (groups[i].attributes !== attribute) throw new Error('mesh\'s primitives have different attributes');
                if ((groups[i].index !== undefined) !== has_index) throw new Error('mesh\'s primitives are partially indexed');
            }
        }
        const merge_group: MeshGroup = {
            position: undefined,
            normal: undefined,
            uv: undefined,
            tangent: undefined,
        ...
        if (has_index) {
            const sum = groups.reduce((sum, cnt) => sum + cnt.index!.length, 0);
            merge_group.index = new Uint32Array(sum);
        }
        const bbox_min = Vector3.create(0, 0, 0);
        const bbox_max = Vector3.create(0, 0, 0);
        if (bitmask_check(attribute, RenderServerGeometryAttributeLayoutBuffer.PositionNormal)) {
            bbox_min.set(+Infinity, +Infinity, +Infinity);
            bbox_max.set(-Infinity, -Infinity, -Infinity);
            const point = GltfLoader.#tmp_vector3_0;
            const sum = groups.reduce((sum, cnt) => sum + cnt.position!.length, 0);
            merge_group.position = new Float32Array(sum * 2);
            let vertex_index = 0;
            let index_index = 0;
            for (const { position, normal, index } of groups) {
                const vertex_count = position!.length / 3;
                if (has_index) {
                    const index_count = index!.length;
                    for (let i = 0; i < index_count; i++) {
                        merge_group.index![index_index + i] = vertex_index + index![i];
                    }
                    surfaces.push({ offset: index_index, length: index_count });
                    index_index += index_count;
                }
                for (let i = 0; i < vertex_count; i++) {
                    const vec6_index = vertex_index * 6;
                    const vec3_index = i * 3;
                    const x = position![vec3_index + 0], y = position![vec3_index + 1], z = position![vec3_index + 2];
                    point.set(x, y, z);
                    bbox_min.min(bbox_min, point);
                    bbox_max.max(bbox_max, point);
                    merge_group.position[vec6_index + 0] = x;
                    merge_group.position[vec6_index + 1] = y;
                    merge_group.position[vec6_index + 2] = z;
                    merge_group.position[vec6_index + 3] = normal === undefined ? 0 : normal[vec3_index + 0];
                    merge_group.position[vec6_index + 4] = normal === undefined ? 0 : normal[vec3_index + 1];
                    merge_group.position[vec6_index + 5] = normal === undefined ? 0 : normal[vec3_index + 2];
                    vertex_index++;
                }
            }
        }
        if (bitmask_check(attribute, RenderServerGeometryAttributeLayoutBuffer.Uv)) {
            const sum = groups.reduce((sum, cnt) => sum + cnt.uv!.length, 0);
            merge_group.uv = new Float32Array(sum);
            let vertex_index = 0;
            for (const { uv } of groups) {
                const vertex_count = uv!.length / 2;
                for (let i = 0; i < vertex_count; i++) {
                    const vertex_vec2_index = vertex_index * 2;
                    const vec2_index = i * 2;
                    merge_group.uv[vertex_vec2_index + 0] = uv![vec2_index + 0];
                    merge_group.uv[vertex_vec2_index + 1] = uv![vec2_index + 1];
                    vertex_index++;
                }
            }
        }
        if (bitmask_check(attribute, RenderServerGeometryAttributeLayoutBuffer.Tangent)) {
            const sum = groups.reduce((sum, cnt) => sum + cnt.tangent!.length, 0);
            merge_group.tangent = new Float32Array(sum);
            let vertex_index = 0;
            for (const { tangent } of groups) {
                const vertex_count = tangent!.length / 3;
                for (let i = 0; i < vertex_count; i++) {
                    const vertex_vec3_index = vertex_index * 3;
                    const vec3_index = i * 3;
                    merge_group.tangent[vertex_vec3_index + 0] = tangent![vec3_index + 0];
                    merge_group.tangent[vertex_vec3_index + 1] = tangent![vec3_index + 1];
                    merge_group.tangent[vertex_vec3_index + 2] = tangent![vec3_index + 2];
                    vertex_index++;
                }
            }
        }
        if (bitmask_check(attribute, RenderServerGeometryAttributeLayoutBuffer.Color)) {
            const sum = groups.reduce((sum, cnt) => sum + cnt.color!.length, 0);
            merge_group.color = new Float32Array(sum);
            let vertex_index = 0;
            for (const { color } of groups) {
                const vertex_count = color!.length / 4;
                for (let i = 0; i < vertex_count; i++) {
                    const vertex_vec4_index = vertex_index * 4;
                    const vec4_index = i * 4;
                    merge_group.color[vertex_vec4_index + 0] = color![vec4_index + 0];
                    merge_group.color[vertex_vec4_index + 1] = color![vec4_index + 1];
                    merge_group.color[vertex_vec4_index + 2] = color![vec4_index + 2];
                    merge_group.color[vertex_vec4_index + 3] = color![vec4_index + 3];
                    vertex_index++;
                }
            }
        }
        ...
        // create geometry
        const geometry = new ArrayGeometry3DResource();
        geometry.clear_Geometry();
        geometry.set_PrimitiveType(WebGPURenderStatePrimitiveType.Triangles);
        geometry.set_BBox(GltfLoader.#tmp_box3_0.set(bbox_min, bbox_max));
    ...
    private readonly mesh_cache: Map<number, { geometry: ArrayGeometry3DResource, materials: number[] }> = new Map();
    private parse_Mesh(index: number) {
        ...
        const geometry = this.create_Mesh(mesh.primitives);
        this.mesh_cache.set(index, geometry);

        return geometry;
    }

    private async parse_Node(index: number): Promise<Node3D> {
        const node = this.json?.nodes?.[index];
        if (node === undefined) throw new Error('node not found');

        const name = node.name;
        const matrix = Matrix4.new;
        if (node.matrix !== undefined) {
            const [
                n11 = 1, n21 = 0, n31 = 0, n41 = 0,
                n12 = 0, n22 = 1, n32 = 0, n42 = 0,
                n13 = 0, n23 = 0, n33 = 1, n43 = 0,
                n14 = 0, n24 = 0, n34 = 0, n44 = 1,
            ] = node.matrix;
            matrix.set(n11, n12, n13, n14, n21, n22, n23, n24, n31, n32, n33, n34, n41, n42, n43, n44);
        }
        else {
            const mat4 = GltfLoader.#tmp_matrix4_0;
            const mat3 = GltfLoader.#tmp_matrix3_0;
            if (node.scale !== undefined) {
                mat3.set_Scale(node.scale[0] ?? 1, node.scale[1] ?? 1, node.scale[2] ?? 1);
                matrix.compose(matrix, mat4.set_Basis(mat3));
            }
            if (node.rotation !== undefined) {
                const quat = GltfLoader.#tmp_quaternion_0;
                quat.set(node.rotation[0] ?? 0, node.rotation[1] ?? 0, node.rotation[2] ?? 0, node.rotation[3] ?? 1);
                mat3.set_Quaternion(quat);
                matrix.compose(matrix, mat4.set_Basis(mat3));
            }
            if (node.translation !== undefined) {
                const vec3 = GltfLoader.#tmp_vector3_0;
                vec3.set(node.translation[0] ?? 0, node.translation[1] ?? 0, node.translation[2] ?? 0);
                matrix.compose(matrix, mat4.set_Position(vec3));
            }
        }
        ...
        if (children.length > 0) {
            ...
            const { geometry, materials } = this.parse_Mesh(node.mesh);
            ...
            mesh_instance.geometry = geometry;
            mesh_instance.material = mat.get();
    ...
    private async parse_Scene(index: number) {
        ...
        if (nodes.length === 1) {
            const n_1 = await nodes[0];
            if (name !== undefined) n_1.name = name;
            return n_1;
        }
        else {
            const root = new Node3D();
            if (name !== undefined) root.name = name;
            const ns = await Promise.all(nodes);
            for (const n_2 of ns) {
                root.add_Child(n_2);
            }
            return root;
    ...
    public async parse(data: string | ArrayBuffer): Promise<Result<Node3D, Error>> {
        let json;
        ...
        if (json.asset === undefined || json.asset.version[0] < 2) {
            return Result.Error(new Error('<GltfLoader> parse: unsupported asset, only glTF versions >= 2.0 are supported'));
        }
        if (buffers === undefined) {
            return Result.Error(new Error('<GltfLoader> parse: unsupported buffer, only glTF with embeded buffers are supported'));
        }

        try {
            this.json = json;
            this.buffers = buffers;
            const scene = await this.parse_Scene(0);
            if (scene === undefined) throw new Error('root scene not found');
            return Result.Ok(scene);
        }
        catch (err) {
            console.error(err);
            return Result.Error(new Error(`<GltfLoader> parse: ${(err as Error).message}`));
        }
    }
}
...
//             }
//             else {
//                 mesh_instance.material = ms.map(m_1 => m_1 === undefined ? undefined : new ThreeMaterialResource(m_1));
//             }
//             if (name !== undefined) mesh_instance.name = name;
//             mesh_instance.local_transform = matrix;
//             return mesh_instance;
...
//     public async parse_Scene(index: number) {
//         const scene = this.json?.scenes?.[index];
//         if (scene === undefined) throw new Error('scene not found');
...
//         if (nodes.length === 0) return Promise.resolve(undefined);
//         if (nodes.length === 1) const n_1 = await nodes[0];
//         if (name !== undefined) n_1.name = name;
//         return n_1;
