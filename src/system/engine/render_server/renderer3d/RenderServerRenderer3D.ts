...
import { ReadonlyRef, Ref, RefArray, RefCacher } from "@/system/utils/RefCounted";
...
import type { PostprocessingResource } from "../../resources/postprocessing_resources/PostprocessingResource";
import { SmaaPostprocessingResource } from "../../resources/postprocessing_resources/smaa_postprocessing_resource/SmaaPostprocessingResource";
import { ToneMapPostprocessingResource } from "../../resources/postprocessing_resources/ToneMapPostprocessingResource";
import { PixelatePostprocessingResource } from "../../resources/postprocessing_resources/PixelatePostprocessingResource";
...
export class RenderServerRenderer3D extends RenderServerObjectRefCounted {
    ...
    protected render_Queue1Effects(encoder: GPUCommandEncoder): number {
        ...
        for (const effect of this.effects) {
            ...
            const count = effect.pass_count;
            for (let pass = 0; pass < count; pass++) {
                const frame_buffer = !flip ? this.effect_frame_buffer_0_ref.expect.frame_buffer_desc : this.effect_frame_buffer_1_ref.expect.frame_buffer_desc;
                const _encoder = encoder.beginRenderPass(frame_buffer);
                const world_env_uniform_group = !flip ? this.world_env_effect_uniform_group_0_ref.expect.binding_group : this.world_env_effect_uniform_group_1_ref.expect.binding_group;
                _encoder.setBindGroup(0, world_env_uniform_group);
                const color_texture_view = this.result_color_render_queue_1_texture_view_ref.expect;
                const normal_texture_view = this.result_normal_render_queue_1_texture_view_ref.expect;
                const depth_texture_view = this.result_depth_render_queue_1_texture_view_ref.expect;
                effect.render_Pass(pass, _encoder, color_texture_view, normal_texture_view, depth_texture_view);
                this.full_screen_triangle_vertex_array_ref.expect.bind_Buffers(_encoder);
                this.full_screen_triangle_vertex_array_ref.expect.draw(_encoder);
                _encoder.end();
                rendered_pass++;
                flip = !flip;
            }
