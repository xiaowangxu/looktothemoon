...
import { RenderServerMaterialPass } from "../material/RenderServerMaterial";
...
export class RenderServerRenderer3D extends RenderServerObjectRefCounted {
    ...
    protected readonly color_texture_ref = new Ref<WebGPURenderStateMultiSampleTexture>();
    protected readonly normal_texture_ref = new Ref<WebGPURenderStateMultiSampleTexture>();
    protected readonly depth_texture_ref = new Ref<WebGPURenderStateMultiSampleTexture>();

    protected readonly color_texture_view_ref = new Ref<WebGPURenderStateTextureView>();
    protected readonly normal_texture_view_ref = new Ref<WebGPURenderStateTextureView>();
    protected readonly depth_texture_view_ref = new Ref<WebGPURenderStateTextureView>();

    protected readonly frame_buffer_ref = new ReadonlyRef(new WebGPURenderElementFrameBuffer(RenderServer.render_state));
    ...
    public resize(width: number, height: number) {
        if (this.texture_size.x !== width || this.texture_size.y !== height) {
            ...
            this.color_texture_ref.value = RenderServer.render_state.create_MultiSampleTexture(WebGPURenderStateTextureUsage.Attchment, WebGPURenderStateTextureFormat.RGBA16F, width, height, WebGPURenderStateMultiSampleCount.MS4).expect();
            this.normal_texture_ref.value = RenderServer.render_state.create_MultiSampleTexture(WebGPURenderStateTextureUsage.Attchment, WebGPURenderStateTextureFormat.RGBA16F, width, height, WebGPURenderStateMultiSampleCount.MS4).expect();
            this.depth_texture_ref.value = RenderServer.render_state.create_MultiSampleTexture(WebGPURenderStateTextureUsage.Attchment, WebGPURenderStateTextureFormat.D32F, width, height, WebGPURenderStateMultiSampleCount.MS4).expect();
            this.color_texture_view_ref.value = RenderServer.render_state.create_TextureView(this.color_texture_ref.expect).expect();
            this.normal_texture_view_ref.value = RenderServer.render_state.create_TextureView(this.normal_texture_ref.expect).expect();
            this.depth_texture_view_ref.value = RenderServer.render_state.create_TextureView(this.depth_texture_ref.expect).expect();
            return true;
        }
        return false;
    }
    ...
    public render(world: World3D, camera: Camera3, viewport: RenderServerViewport, time: number, once: boolean) {
        ...
        if (this.resize(texture_size.x, texture_size.y) || viewport.id !== this.last_viewport_id) {
            this.frame_buffer_ref.expect.clear_Attachments();
            this.frame_buffer_ref.expect.clear_DepthStencilAttachment();
            this.frame_buffer_ref.expect.add_Attachment(this.color_texture_view_ref.expect, true, Vector4.create(0.2, 0.2, 0.2, 1), true, viewport.canvas_texture_view);
            this.frame_buffer_ref.expect.add_Attachment(this.normal_texture_view_ref.expect, true, Vector4.create(0, 0, 0, 1), true);
            this.frame_buffer_ref.expect.set_DepthStencilAttachment(this.depth_texture_view_ref.expect, true, 1, true);
        }
        this.last_viewport_id = viewport.id;
        viewport.update_Size();
        this.frame_buffer_ref.expect.refresh_CanvasTextureView();
        ...
        const command_encoder = RenderServer.render_state.device.createCommandEncoder();
        const render_pass = command_encoder.beginRenderPass(this.frame_buffer_ref.expect.frame_buffer_desc);
        ...
        for (let i = 0; i <= this.queue_0.solid_pointer; i++) {
            ...
            const pipeline_uniform = material.get_PipelineUniform(RenderServerMaterialPass.Solid, vertex_array, this.frame_buffer_ref.expect, WebGPURenderStateDepthCompareFunc.LessEqual);
        ...
        render_pass.end();
        RenderServer.render_state.device.queue.submit([command_encoder.finish()]);
    ...
    public fill_Queue(meshes: Iterable<VisualWorld3DMesh>, camera: Camera3, camera_frustum: Frustum3, screen_size: Vector2) {
    ...
    public dispose(): void {
        ...
        this.color_texture_ref.clear();
        this.depth_texture_ref.clear();
        this.color_texture_view_ref.clear();
        this.depth_texture_view_ref.clear();
