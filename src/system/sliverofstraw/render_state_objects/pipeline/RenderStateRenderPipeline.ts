...
import type { RenderStateAttributeLayout } from "./RenderStateAttributeLayout";

export abstract class RenderStateRenderPipeline<T extends RenderState<T>> extends RenderStateObjectRefCounted<T> {

    public readonly program_ref: Ref<RenderStateProgram<T>> = new Ref();
    public readonly program_state: RenderStateProgramState<T>;

    constructor(render_state: T, program: RenderStateProgram<T>, program_state: RenderStateProgramState<T>, uniform_layouts: Iterable<RenderStateUniformLayout<T>>, attribute_layouts: Iterable<RenderStateAttributeLayout>) {
        super(render_state);
