...
import { RenderStateBufferUsage, RenderStatePrimitiveType, RenderStateShaderType, RenderStateTextureFormat, RenderStateTextureType, RenderStateTextureWrap, RenderStateValueType } from "./RenderState";
...
const uniforms = { u_texture: { type: RenderStateValueType.Tex2D } };
...
const f_fragmentShaderSource2 = process_WebGL2ShaderCode(RenderStateShaderType.Fragment,
	attributes, uniforms, varyings, outputs,
	`o_color = vec4(v_world * texture(u_texture, v_uv / (sin(time) + 1.5)).rgb, 1.0);`
);
...
const material = new Ref(new WebGL2RenderDeviceMaterialSet(render_device, vert_shader, { default: frag_shader, test: frag_shader2 }));
...
const u_texture_location = render_device.render_state.get_ProgramUniformLocation(material.expect.get_Program('test')!, 'u_texture');
console.log(u_texture_location);

render_device.render_state.set_CapabilityProxy(render_device.render_state.gl.DEPTH_TEST, true);
render_device.render_state.active_Texture(texture, 2);
...
function render(time: number) {
	...
	render_state.use_ProgramProxy(material.expect.get_Program('test')!.program);
	render_state.gl.uniform1i(u_texture_location, 2);
